...
## needs to be made scalabe to run with for_each statement based on the linux_vm_debian module count
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      --command 'if [ ! -f /tmp/haproxy-poc/startup_finished.txt ]; then exit 0; else cd /tmp/haproxy-poc/ && sudo git pull && sudo ansible-playbook /tmp/haproxy-poc/playbooks/deploy.yml && echo "Playbook executed successfully." || { echo "Playbook execution failed"; exit 1; }; fi'
    EOT
  }
  triggers = {
